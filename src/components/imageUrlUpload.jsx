...
const LatestImageDisplay = () => {
  ...
  const fetchLatestImage = async () => {
    try {
      ...
      setLatestImage(sortedImages[0] || null);
    } catch (error) {
  ...
  const handleDelete = async (imageId) => {
    ...
    try {
      setDeleting(true);
      await deleteDoc(doc(db, "coverImage", imageId));
      setLatestImage(null);
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, "coverImage"), {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-xl font-semibold mb-4">Latest Uploaded Image</h2>
        ...
        {fetchingImage ? (
          ...
          <div className="relative rounded-lg overflow-hidden shadow-md">
            <img
              src={latestImage.imageData || latestImage.imageUrl}
              alt={latestImage.fileName || "Latest uploaded image"}
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white p-2 text-sm flex justify-between items-center">
              <span>
                {new Date(latestImage.uploadDate).toLocaleDateString()}
              </span>
              <button
...
export default LatestImageDisplay;
